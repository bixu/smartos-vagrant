...
EOF
  ...
  vmadm create -f /zones/zone.json || (echo "Zone creation didn't quite work. Fix manually and press enter..."; read)
...
RemoveImage() {
  if zfs list zones/$ZONE_IMAGE >/dev/null 2>&1; then
    ...
    for i in $(zfs  list -t snapshot -Ho name); do
	zfs destroy $i
    done
...
groupadd -g \${USERID} vagrant
useradd -g \${USERID} -u \${USERID} -m -d /home/vagrant -s /bin/bash vagrant
...
EOKEY
chown -R \${USERID}:\${USERID} ~vagrant/.ssh
